...
export async function startBot() {
  ...
  client.on(Events.InteractionCreate, async interaction => {try{
    ...
    if(interaction.isChatInputCommand()){if(await handleBillingCommand(client,interaction))return;if(await handleEconomyCommand(client,interaction))return;if(await handleTicketOps(client,interaction))return;if(await handleFeatureCommand(client,interaction))return;await handleCommand(client,interaction);}
  ...
  client.on(Events.MessageCreate,async message=>{await handleMessage(message);if(message.deleted)return;await handleCommunityMessage(message).catch(console.error);await onMemberActivity(message).catch(console.error);await onEconomyMessage(message).catch(console.error);});
  client.on(Events.MessageReactionAdd,(reaction,user)=>{void onCommunityReactionAdd(reaction,user);});
  client.on(Events.MessageReactionRemove,(reaction,user)=>{void onCommunityReactionRemove(reaction,user);});
  client.on(Events.GuildMemberAdd,async member=>{await onMemberJoinLeave(member.guild.id,"joins").catch(console.error);await onEconomyJoin(member.guild.id,member.id).catch(console.error);await handleJoin(client,member);const feature=await getFeature(member.guild.id,"welcome",{channelId:"",autoRoleId:"",message:"Welcome {user} to {server}!",dmWelcome:false});if(!feature.enabled)return;if(feature.config.autoRoleId)await member.roles.add(String(feature.config.autoRoleId)).catch(()=>{});const text=String(feature.config.message||"Welcome {user}!").replaceAll("{user}",`<@${member.id}>`).replaceAll("{server}",member.guild.name);if(feature.config.channelId){const ch=await client.channels.fetch(String(feature.config.channelId)).catch(()=>null);if(ch?.isTextBased())await (ch as TextChannel).send(text).catch(()=>{});}if(feature.config.dmWelcome)await member.send(text.replace(`<@${member.id}>`,member.user.username)).catch(()=>{});});
