...
app.get("/health",(_req,res)=>res.json({ok:true,discordReady:client.isReady(),time:new Date().toISOString()}));
...
app.get("/dashboard",requireAuth,async(req,res)=>{
  ...
  const stats=await one<any>(`SELECT
    ...
    (SELECT count(*) FROM warnings WHERE guild_id=$1) warnings`,[config.targetGuildId]);
...
app.get("/modules/:key",requireAuth,async(req,res)=>{
  const def=moduleMap.get(req.params.key);
  if(!def) return res.status(404).send("Unknown module");
...
app.post("/modules/:key",requireAuth,async(req,res)=>{
  ...
  if(!def) return res.status(404).send("Unknown module");
  let parsed:any;
  try { parsed=JSON.parse(req.body.config||"{}"); } catch { return res.status(400).send("Config must be valid JSON."); }
  const enabled=req.body.enabled==="on";
  await query(`INSERT INTO feature_settings(guild_id,feature_key,enabled,config,updated_at) VALUES($1,$2,$3,$4::jsonb,now())
    ON CONFLICT(guild_id,feature_key) DO UPDATE SET enabled=$3,config=$4::jsonb,updated_at=now()`,
    [config.targetGuildId,def.key,enabled,JSON.stringify(parsed)]);
...
app.get("/social",requireAuth,async(req,res)=>{
  const feeds=await query<any>(`SELECT * FROM social_feeds WHERE guild_id=$1 ORDER BY id DESC`,[config.targetGuildId]);
  const ui=await getGuildUi();
  res.render("social",{user:req.session.user,feeds,...ui,baseUrl:config.baseUrl});
});
app.post("/social",requireAuth,async(req,res)=>{
  const provider=String(req.body.provider||"rss");
  const secret=provider==="webhook"?createWebhookSecret():null;
  await query(`INSERT INTO social_feeds(guild_id,name,provider,source,channel_id,enabled,include_keywords,exclude_keywords,mention_role_id,secret_key)
    VALUES($1,$2,$3,$4,$5,true,$6,$7,$8,$9)`,[
      config.targetGuildId,String(req.body.name||"Feed"),provider,String(req.body.source||""),String(req.body.channelId||""),
      String(req.body.includeKeywords||"").split(",").map((s:string)=>s.trim()).filter(Boolean),
      String(req.body.excludeKeywords||"").split(",").map((s:string)=>s.trim()).filter(Boolean),
      req.body.mentionRoleId||null,secret
    ]);
  await audit(config.targetGuildId,req.session.user!.id,"social.create",{provider,name:req.body.name});
  res.redirect("/social");
});
app.post("/social/:id/toggle",requireAuth,async(req,res)=>{
  await query(`UPDATE social_feeds SET enabled=NOT enabled,updated_at=now() WHERE id=$1 AND guild_id=$2`,[req.params.id,config.targetGuildId]);
  res.redirect("/social");
});
app.post("/social/:id/delete",requireAuth,async(req,res)=>{
  await query(`DELETE FROM social_feeds WHERE id=$1 AND guild_id=$2`,[req.params.id,config.targetGuildId]);
  await audit(config.targetGuildId,req.session.user!.id,"social.delete",{id:req.params.id});
  res.redirect("/social");
});

app.post("/hooks/social/:secret",async(req,res)=>{
  const ok=await deliverWebhook(req.params.secret,req.body);
  res.status(ok?202:404).json({ok});
});

app.get("/moderation",requireAuth,async(req,res)=>{
  const warnings=await query<any>(`SELECT * FROM warnings WHERE guild_id=$1 ORDER BY created_at DESC LIMIT 100`,[config.targetGuildId]);
  const auditRows=await query<any>(`SELECT * FROM audit_log WHERE guild_id=$1 ORDER BY created_at DESC LIMIT 100`,[config.targetGuildId]);
  res.render("moderation",{user:req.session.user,warnings,auditRows});
});

app.get("/trading",requireAuth,async(req,res)=>{
  const calls=await query<any>(`SELECT * FROM trade_calls WHERE guild_id=$1 ORDER BY created_at DESC LIMIT 100`,[config.targetGuildId]);
  res.render("trading",{user:req.session.user,calls});
});
app.post("/trading/:id/status",requireAuth,async(req,res)=>{
  const status=String(req.body.status||"LIVE");
  if(!["LIVE","HIT","PROFIT","MISS","EXPIRED"].includes(status)) return res.status(400).send("Invalid status");
  await query(`UPDATE trade_calls SET status=$1,closed_at=CASE WHEN $1='LIVE' THEN NULL ELSE now() END WHERE id=$2 AND guild_id=$3`,[status,req.params.id,config.targetGuildId]);
  await audit(config.targetGuildId,req.session.user!.id,"trade.status",{id:req.params.id,status});
  res.redirect("/trading");
});

app.get("/tickets",requireAuth,async(req,res)=>{
  const tickets=await query<any>(`SELECT * FROM tickets WHERE guild_id=$1 ORDER BY created_at DESC LIMIT 100`,[config.targetGuildId]);
  res.render("tickets",{user:req.session.user,tickets});
});
app.post("/tickets/:id/close",requireAuth,async(req,res)=>{
  const ticket=await one<any>(`SELECT * FROM tickets WHERE id=$1 AND guild_id=$2`,[req.params.id,config.targetGuildId]);
  if(ticket?.channel_id) {
    const ch=await client.channels.fetch(ticket.channel_id).catch(()=>null);
    if(ch) await (ch as any).delete(`Ticket closed from dashboard by ${req.session.user!.username}`).catch(()=>{});
  }
  await query(`UPDATE tickets SET status='CLOSED',closed_at=now() WHERE id=$1 AND guild_id=$2`,[req.params.id,config.targetGuildId]);
  await audit(config.targetGuildId,req.session.user!.id,"ticket.close",{id:req.params.id});
  res.redirect("/tickets");
});

app.get("/audit",requireAuth,async(req,res)=>{
  const rows=await query<any>(`SELECT * FROM audit_log WHERE guild_id=$1 ORDER BY created_at DESC LIMIT 250`,[config.targetGuildId]);
  res.render("audit",{user:req.session.user,rows});
});
