...
async function questRows(guildId:string,userId:string){
  const [season,premium]=await Promise.all([
    one<any>(`SELECT id FROM economy_seasons WHERE guild_id=$1 AND active=true ORDER BY starts_at DESC LIMIT 1`,[guildId]),
    one<any>(`SELECT 1 FROM entitlements WHERE guild_id=$1 AND discord_user_id=$2 AND active=true AND (expires_at IS NULL OR expires_at>now()) LIMIT 1`,[guildId,userId])
  ]);
  ...
  const out=[] as any[];for(const q of defs){if(q.premium_only&&!premium)continue;const key=questPeriodKey(q.cadence,season?.id);const p=await one<any>(`SELECT * FROM member_quest_progress WHERE guild_id=$1 AND user_id=$2 AND quest_id=$3 AND period_key=$4`,[guildId,userId,q.id,key]);out.push({...q,period_key:key,progress:Number(p?.progress||0),rewarded:Boolean(p?.rewarded_at)});}return out;
}
