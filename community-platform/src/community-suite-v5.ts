...
export async function onV5AuditEntry(entry:any,guild:any){
  ...
  await query(`INSERT INTO security_events(guild_id,event_type,actor_id,target_id,severity,details,action_taken) VALUES($1,'destructive_action',$2,$3,$4,$5::jsonb,$6)`,[guild.id,actor.id,entry.targetId||null,count>limit?"critical":"warning",JSON.stringify({action:actionType,count,limit}),actionTaken]);
}
...
export async function scanHealth(client:Client,guildId:string){
  ...
  const deadCounters=await query<any>(`SELECT id,channel_id FROM server_counters WHERE guild_id=$1 AND enabled=true`,[guildId]);for(const c of deadCounters)if(!guild.channels.cache.has(c.channel_id))push(`counter-channel:${c.id}`,"warning","Server counter points to a deleted channel","Disable the broken counter.","disable_counter",{id:c.id});
