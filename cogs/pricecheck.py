...
class PriceCheck(commands.Cog):
    ...
    def generate_price_graph(self, ps_data, player_id):
        try:
            times = [datetime.utcfromtimestamp(t[0] / 1000) for t in ps_data]
            prices = [t[1] for t in ps_data]

            plt.figure(figsize=(10, 4))
            plt.plot(times, prices, marker='o', linestyle='-', color='blue', label="Cross-play Price")
            plt.title("FUTBIN Price Trend (Cross-play)")
            plt.xlabel("Time")
            plt.ylabel("Coins")
            plt.grid(True)
            ...
            file_path = f"price_graph_{player_id}.png"
    ...
    async def pricecheck(self, interaction: discord.Interaction, player: str, platform: str = "console"):
        ...
        try:
            ...
            ps_data = json.loads(graph_div.get("data-ps-data"))
            graph_path = self.generate_price_graph(ps_data, match["id"])
        except Exception as e:
            log.warning(f"[GRAPH DATA MISSING] {e}")
        ...
        embed.add_field(name="📈 Graph", value=f"[View Full Graph]({url}#player-stats-graph)", inline=False)
