...
class LeakTweets(commands.Cog):
        ...
        for username, feed_url in self.feeds.items():
            try:
                ...
                for entry in feed.entries:
                    ...
                    if tweet_link in self.seen.get(username, []):
                        continue

                    self.seen.setdefault(username, []).append(tweet_link)
    ...
    @app_commands.describe(channel="Channel to post leaks in", role="Optional role to ping")
    async def preloadfeeds(self, interaction: discord.Interaction, channel: discord.TextChannel, role: discord.Role = None):
        ...
        await interaction.response.send_message(f"✅ Default feeds loaded into {channel.mention}", ephemeral=True)

    @app_commands.command(name="setleakchannel", description="📍 Set the channel (and ping role) for FUT leak tweets.")
